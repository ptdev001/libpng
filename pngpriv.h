 ...
 */
  ...
  #define PNG_INTERNAL_CALLBACK(type, name, args, attributes) \
    PNG_LINKAGE_CALLBACK PNG_FUNCTION(type, (PNGCBAPI name), args, PNG_EMPTY attributes)
#endif
 ...
 */
   ...
   */
    ...
    #if (defined(__MWERKS__) && defined(macintosh)) || defined(applec) || defined(THINK_C) || defined(__SC__) || \
...
#define PNG_COLOR_DIST(c1, c2)                                                           \
  (abs((int)((c1).red) - (int)((c2).red)) + abs((int)((c1).green) - (int)((c2).green)) + \
...
#define PNG_ROWBYTES(pixel_bits, width)                                          \
  ((pixel_bits) >= 8 ? ((png_size_t)(width) * (((png_size_t)(pixel_bits)) >> 3)) \
 ...
 */
   ...
   */
    ...
    #define png_fixed(png_ptr, fp, s) \
      ((fp) <= 21474 && (fp) >= -21474 ? ((png_fixed_point)(100000 * (fp))) : (png_fixed_error(png_ptr, s), 0))
  #endif
...
PNG_INTERNAL_DATA(const png_byte, png_sRGB_delta, [512]);
    ...
    #define PNG_sRGB_FROM_LINEAR(linear)                                                                             \
      ((png_byte)(0xff &                                                                                             \
                  ((png_sRGB_base[(linear) >> 15] + ((((linear)&0x7fff) * png_sRGB_delta[(linear) >> 15]) >> 12)) >> \
                   8)))
...
extern "C"
{
  ...
  PNG_INTERNAL_FUNCTION(void, png_free_buffer_list, (png_structrp png_ptr, png_compression_bufferp* list), PNG_EMPTY);
      ...
      (defined(PNG_gAMA_SUPPORTED) || defined(PNG_cHRM_SUPPORTED) || defined(PNG_sCAL_SUPPORTED) || \
       defined(PNG_READ_BACKGROUND_SUPPORTED) || defined(PNG_READ_RGB_TO_GRAY_SUPPORTED)) ||        \
  ...
  PNG_INTERNAL_FUNCTION(void,
                        png_write_tEXt,
                        (png_structrp png_ptr, png_const_charp key, png_const_charp text, png_size_t text_len),
                        PNG_EMPTY);
  #endif
  ...
  PNG_INTERNAL_FUNCTION(int, png_zlib_inflate, (png_structrp png_ptr, int flush), PNG_EMPTY);
  ...
  PNG_INTERNAL_FUNCTION(void,
                        ...
                        (png_structrp png_ptr, png_bytep buffer, png_size_t buffer_length),
                        PNG_EMPTY);
  ...
  PNG_INTERNAL_FUNCTION(
    void,
    png_ascii_from_fp,
    (png_const_structrp png_ptr, png_charp ascii, png_size_t size, double fp, unsigned int precision),
    PNG_EMPTY);
  ...
  PNG_INTERNAL_FUNCTION(void,
                        ...
                        (png_const_structrp png_ptr, png_charp ascii, png_size_t size, png_fixed_point fp),
                        PNG_EMPTY);
    #endif /* FIXED_POINT */
  #endif   /* sCAL */
  ...
  PNG_INTERNAL_FUNCTION(int,
                        png_check_fp_number,
                        (png_const_charp string, png_size_t size, int* statep, png_size_tp whereami),
                        PNG_EMPTY);
